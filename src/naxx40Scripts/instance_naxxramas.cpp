            ...
            if (state == DONE)
            {
                ...
                switch (bossId)
                {
                    ...
                    case BOSS_LOATHEB:
                        ...
                        events.ScheduleEvent(EVENT_KELTHUZAD_WING_TAUNT, 6000ms);
                        break;
                    case BOSS_ANUB:
                    ...
                    case BOSS_GOTHIK:
                        ...
                        break;
                    case BOSS_SAPPHIRON:
                        events.ScheduleEvent(EVENT_FROSTWYRM_WATERFALL_DOOR, 5000ms);
                        break;
                    case BOSS_THADDIUS:
    ...
    struct boss_naxxramas_miscAI : public NullCreatureAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (me->GetEntry() == NPC_NAXXRAMAS_TRIGGER)
            {
                ...
                if (timer >= 5000)
                {
                    if (Creature* cr = me->SummonCreature(NPC_LIVING_POISON, 3128.59, -3118.81, 293.346, 4.76754, TEMPSUMMON_TIMED_DESPAWN, 15200))
                    {
                        ...
                        cr->GetMotionMaster()->MovePoint(0, 3130.322, -3156.51, 293.324, FORCED_MOVEMENT_NONE, 0.0f, 0.0f, false);
                    }
                    if (Creature* cr = me->SummonCreature(NPC_LIVING_POISON, *me, TEMPSUMMON_TIMED_DESPAWN, 14800))
                    {
                        cr->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                        cr->GetMotionMaster()->MovePoint(0, 3144.779, -3158.416, 293.324, FORCED_MOVEMENT_NONE, 0.0f, 0.0f, false);
                    }
                    if (Creature* cr = me->SummonCreature(NPC_LIVING_POISON, 3175.42, -3134.86, 293.34, 4.284, TEMPSUMMON_TIMED_DESPAWN, 14800))
                    {
                        cr->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                        cr->GetMotionMaster()->MovePoint(0, 3158.778, -3164.201, 293.312, FORCED_MOVEMENT_NONE, 0.0f, 0.0f, false);
                    }
                    timer = 0;
                }
            }
            else if (me->GetEntry() == NPC_LIVING_POISON)
            {
                ...
                Cell::VisitObjects(me, searcher, 1.5f);
                if (target)
                {
