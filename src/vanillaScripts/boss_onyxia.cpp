        ...
        void SetPhase(uint8 ph)
        {
            ...
            switch (ph)
            {
                ...
                case PHASE_AIRPHASE:
                    ...
                    break;
                case PHASE_LANDED:
                    events.ScheduleEvent(EVENT_START_PHASE_3, 5000ms);
                    break;
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id < 9)
            {
                if (id > 0 && Phase == PHASE_AIRPHASE)
                {
                    ...
                    CurrentWP = id;
                    events.ScheduleEvent(EVENT_SPELL_FIREBALL_FIRST, 1000ms);
                }
            }
            else
            {
        ...
        if (!UpdateVictim() || !CheckInRoom())
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_SPELL_CLEAVE:
                {
                    ...
                    events.Repeat(Milliseconds(urand(2000, 5000)));
                    break;
                }
                ...
                case EVENT_LIFTOFF:
                {
                    ...
                    events.RescheduleEvent(EVENT_END_MANY_WHELPS_TIME, 10000ms);
                    break;
                }
                ...
                case EVENT_FLY_S_TO_N:
                {
                    ...
                    whelpSpam = true;
                    events.ScheduleEvent(EVENT_WHELP_SPAM, 90000ms);
                ...
                case EVENT_SPELL_FIREBALL_FIRST:
                {
                    ...
                    events.ScheduleEvent(EVENT_SPELL_FIREBALL_SECOND, 4000ms);
                    break;
                }
                case EVENT_SPELL_FIREBALL_SECOND:
                {
                    ...
                    if (rand < 33)
                    {
                        events.ScheduleEvent(EVENT_PHASE_2_STEP_CW, 4000ms);
                    }
                    else if (rand < 66)
                    {
                        events.ScheduleEvent(EVENT_PHASE_2_STEP_ACW, 4000ms);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_PHASE_2_STEP_ACROSS, 4000ms);
                    }
                    break;
                }
